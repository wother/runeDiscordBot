...
func ParseMessage(message string) map[string]interface{} {
	...
	return parseVerb(inputMessageArr, r)
}
...
func parseVerb(inputStringArr []string, r *rand.Rand) map[string]interface{} {
	...
	case strings.HasPrefix(verb, "cast"), strings.HasPrefix(verb, "draw"), strings.HasPrefix(verb, "rune"):
		if len(inputStringArr) > 1 {
			...
			if stringInSlice(numStr, numberStrings) {
				...
				if num == 1 {
					output["type"] = "embed"
					output["content"] = RandomRune(1, r)
				} else {
					output["type"] = "runeArray"
					output["content"] = RandomRune(num, r)
				}
			}
		} else {
			output["type"] = "embed"
...
func getRune(inputString string, infoBoolean bool, r *rand.Rand) map[string]interface{} {
	output := make(map[string]interface{})
	if stringInSlice(inputString, numberStrings) {
		if inputString == "one" {
			output["content"] = RandomRune(NumStringToInt(inputString), r)
			output["type"] = "embed"
		} else {
			output["content"] = RandomRune(NumStringToInt(inputString), r)
			output["type"] = "runeArray"
